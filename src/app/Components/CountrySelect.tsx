...
import { Select, SelectContent, SelectTrigger, SelectItem, SelectValue } from "./lightswind/select";
...
function countryCodeToEmoji(code: string) {
    // Convert ISO-2 code to regional indicator symbols to get flag emoji
    // If code invalid return empty string
    if (!code || code.length !== 2) return "";
    const A = 65; // 'A'.charCodeAt(0)
    const cc = code.toUpperCase();
    return String.fromCodePoint(
        cc.charCodeAt(0) - A + 0x1f1e6,
        cc.charCodeAt(1) - A + 0x1f1e6
    );
}
    ...
    return (
        <Select value={value ?? ""} onValueChange={(value) => { onChange(value) }} >
            ...
            <SelectContent className="bg-black/80">
                {COUNTRIES.map((c) => (
                    <SelectItem key={c.code} value={c.name}>{countryCodeToEmoji(c.code)} {c.name}</SelectItem>
                ))}
            </SelectContent>
        </Select>
    );
